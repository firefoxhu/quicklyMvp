...
public class RetrofitFactory {
    ...
    private static final Interceptor cacheControlInterceptor = chain -> {
        ...
        if (!NetWorkUtil.isNetworkConnected(QuicklyApplication.appContext)) {
    ...
    public static Retrofit getRetrofit() {
        if (retrofit == null) {
            synchronized (RetrofitFactory.class) {
                if (retrofit == null) {
                    // 指定缓存路径,缓存大小 50Mb
                    Cache cache = new Cache(new File(QuicklyApplication.appContext.getCacheDir(), "HttpCache"),
                            1024 * 1024 * 50);

                    // Cookie 持久化
                    ClearableCookieJar cookieJar =
                            new PersistentCookieJar(new SetCookieCache(), new SharedPrefsCookiePersistor(QuicklyApplication.appContext));
                    ...
                    retrofit = new Retrofit.Builder()
                            .baseUrl("http://39.107.228.75:9001/")
